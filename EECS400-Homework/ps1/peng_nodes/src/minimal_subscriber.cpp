...
{ 
  ...
  // it wakes up every time a new message is published on "vel_cmd" 
...
{ 
  ...
  //create a Subscriber object and have it subscribe to the topic "vel_cmd" 
  // the function "myCallback" will wake up whenever a new message is published to vel_cmd 
  // the real work is done inside the callback function 
  
  ros::Subscriber my_subscriber_object= n.subscribe("vel_cmd",1,myCallback); 
