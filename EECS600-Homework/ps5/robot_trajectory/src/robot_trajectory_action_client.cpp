// robot_trajectory_action_client: 
// see complementary server, "robot_trajectory_action_server"
...
void doneCb(const actionlib::SimpleClientGoalState& state,
        const robot_trajectory::TrajActionResultConstPtr& result) {
    ROS_INFO(" doneCb: server responded with state [%s]", state.toString().c_str());
}

int main(int argc, char** argv) {
    ros::init(argc, argv, "demo_trajectory_client_node"); // name this node 
    robot_trajectory::TrajActionGoal goal; //instantiate a goal message compatible with our server, as defined in this package
	...
	double final_phase = 10*3.1415927; // radians--x periods
    ...
    actionlib::SimpleActionClient<robot_trajectory::TrajActionAction> action_client("robot_traj_action_server", true);
	...
	for (phase=0.0;phase<final_phase;phase+=omega*dt) {
        ...
        trajectory_point.positions[1] = -2*q_des; // do this for every joint, from 0 through njnts-1
        trajectory_point.velocities[1] = -2*qdot_des; // and all velocities (in the server example, velocities will get ignored)
